...
Vagrant.configure(2) do |config|
    ...
    #config for app
    config.vm.define :rplcr do |rplcr|

        #network
        rplcr.vm.network "forwarded_port", guest: 8282, host: 8282, auto_correct: true
        rplcr.vm.network "forwarded_port", guest: 9000, host: 9000, auto_correct: true
        rplcr.vm.network "public_network"

        #shared folders
        rplcr.vm.synced_folder ".", "/rplcr"
        rplcr.vm.synced_folder ".", "/vagrant", disabled: true

        #scripts
        rplcr.vm.provision :shell, inline: $install_docker_compose
        rplcr.vm.provision :shell, inline: $run_docker_compose, run: "always", :privileged => false
